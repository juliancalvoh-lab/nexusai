...
def explain_example(row: pd.Series, p: float, thr: float, explainer: Pipeline,
    ...
    if str(row.get("source", "")).endswith("+noise"):
        facts.append("**this record carries an injected label flip**, so the model's "
                     "prediction is arguably correct and the *label* is wrong")

    # Derive the explanation from this row's actual top driver rather than a
    # canned sentence, and say nothing at all when the label itself was flipped.
    flipped = str(row.get("source", "")).endswith("+noise")
    if flipped:
        why = ""
    elif dense_idx:
        lead = names[dense_idx[0]].split("__", 1)[-1]
        lv = raw_row[pos_of[names[dense_idx[0]]]] if names[dense_idx[0]] in pos_of else float("nan")
        val = f"={lv:g}" if lv == lv else ""
        why = (f"`{lead}`{val} carried the most weight here."
               if kind == "FP" else
               f"`{lead}`{val} was the strongest signal and it pointed the wrong way.")
    elif text_drivers:
        why = "The dense features were near neutral, so the text block decided it."
    else:
        why = "No single feature dominates. The score sits close to the threshold."
...
- {ds['label_noise'] * 100:.1f}% symmetric label noise is injected on purpose, so the
