...
Legitimate mail that would get quarantined.
...
- **id** `syn-002686`  |  **theme** `password_reset`  |  **score** 0.557 (threshold 0.272)
  ...
  - **Why it fired:** Legitimate message that happens to share surface cues the model leans on.
...
- **id** `syn-006020`  |  **theme** `legal_threat`  |  **score** 0.009 (threshold 0.272)
  ...
  - **Why it was missed:** None of the cues the model leans on are present, so the score stayed low.
...
24 of 54 errors (44.4%) fall on records whose label the synthetic noise process flipped. On those the model is arguably right and the label is wrong. That noise is why a score near 1.00 would mean something is broken. The other 30 are genuine model failures.
...
- **id** `syn-000725`  |  **theme** `shipping`  |  **score** 0.999 (threshold 0.272)
  ...
  - **Signals:** 1/3 auth checks passed (SPF=1, DKIM=0, DMARC=0); 3 URL(s); reply-to header present; **this record carries an injected label flip** (3% synthetic annotation noise) - the model's prediction is arguably correct and the *label* is wrong
