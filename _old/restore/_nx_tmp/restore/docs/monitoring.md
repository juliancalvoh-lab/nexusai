...
revoke, but it takes 2 days, which is why the pause comes first.
...
reporter is an outlier, investigate that reporter before clearing. Do not clear
the circuit just to unblock sales.
...
why it didn't. Purchases are blocked meanwhile, which is the safe direction.

**Elevated disputes.** Pull the reasons off `DisputeOpened`. If they cluster on
one model, delist it pending investigation. If they're spread, look for a
mechanism problem before assuming bad providers.
...
`MAX_EPOCH_EMISSION`, but not past it, and that's a 2-day proposal, so notice
early.
