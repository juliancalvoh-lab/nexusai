...
double its demand density to break even in year three. Raising the fee will not
fix that.
...
There are benefits the expected-value calculation doesn't capture. The accuracy
floor stops the buyer paying once the model stops working, instead of waiting for
renewal. A bad month is refundable through escrow, and the provider bond puts the
vendor's own money at risk, which a normal software contract does not do.
...
out terminal value. On these numbers I would ask for phase-one funding and
nothing more.
