...
exceeded globally. Governance and staking only exist on the hub, because two governors on two
chains would conflict.
...
(default 1 day) before it returns a score. It publishes the median.
...
submit accuracy in basis points. The contract takes the median rather than the
mean so one extreme report cannot drag the result.
...
`ICrossChainRouter`. Everything above the interface stays as it is.
...
1. No cross-chain licence revocation. A buyer-favourable dispute on the hub
   ...
   a revocation. I did not have time to build that, so the gap is still there.
2. Rewards that accrue while the vault is empty are stranded. The accumulator
   ...
   distributed. No one can withdraw it.
